<?php
	...
	function increment_views_counter($entity_guid) {
        ...
        'annotation_owner_guid' => $user->guid));
			...
			if ($views_counter) {
				return update_annotation($views_counter[0]->id,'views_counter',$views_counter[0]->value + 1,'integer',$user_guid,2);
			} else {
				return create_annotation($entity->guid,'views_counter',1,'integer',$user_guid,2);
			}
		}
	}
	...
	function list_entities_by_most_viewed($options) {
		...
		$count = get_entities_by_most_viewed(array_merge(array('count' => TRUE), $options));
